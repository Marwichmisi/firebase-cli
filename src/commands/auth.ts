...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  firebase-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  firebase-cli auth show\n  firebase-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: firebase-cli auth set <token>");
      return;
    }
...
authCommand
  ...
  .action(async () => {
    try {
      await client.get("/v1beta1/projects", { pageSize: "1" });
